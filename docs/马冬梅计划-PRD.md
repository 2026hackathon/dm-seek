...
| 文档版本 | v0.2（已回填用户对 10 个开放问题的答复，待用户最终确认） |
| 日期 | 2026-06-12 |
| 负责人 | architect |
...
- **语言：中英双语**——报告同时产出**中文与英文**两个版本（由 dongmei-ma 在交付环节负责双语呈现）。
...
| # | id | 职责 | 信息源 / 归属 | 阶段 |
| --- | --- | --- | --- | --- |
| 1 | `dongmei-ma` | 编排、用户接口、拆解任务、驱动校验返工循环、最终**中英双语**交付 | 编排层，不直连信息源 | 首版 |
...
- **输出语言已定为中英双语**（报告同时产出中文与英文，见 4.3）——原「全程中文」假设已被 O9 结论取代。
...
| O9 | 输出语言 | **中英双语**（报告同时产出中文与英文） | 4.3、6.1（dongmei-ma）、11.2 |
...
- **双语成本**：每份报告产出中英两版会增加 token/时间开销，KB 沉淀的语言形式需在设计阶段明确（沉淀一种还是两种）。
