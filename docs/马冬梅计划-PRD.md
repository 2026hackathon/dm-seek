...
| 文档版本 | v0.4.4（承 v0.4.3；本版修订：**L1 降级为软边界**——6/16 全项目评审实证 L1 tools 白名单在 agent body > ~40 行时不生效，dm-seek 全部 7 agent body 均超此阈值；两道防线（软边界）诚实声明同步修订（§6.2 / §6.4 / §9 / §10.1 / §11.1 / §11.4 / 附录A / 附录B） |
| 日期 | 2026-06-17 |
| 负责人 | architect |
| 状态 | v0.4.4（L1 软边界一致性修订），待 critic 审查 |
...
- 该形态下，**MCP 与 skills 从项目级共享配置加载**（共享 `.mcp.json`、项目级 `.claude/skills/`），而非写在各 agent frontmatter 的 `mcpServers` / `skills` 字段——后者在 teammate 形态下**不生效**（详见附录 B）。因此「独占」是**边界约束（软边界：声明层 + 校验层两防线）**而非物理隔离；L1 tools 白名单在 agent body > ~40 行时不生效（dm-seek 全部 7 agent 均超此阈值），已降级为软边界（详见 §6.2、§6.4、§11.1）。
...
>
> **运行形态**：7 个 agent 是**平级 teammate**（非 subagent 父子委派），经共享任务列表 + 消息协作。下表「职责域 / 允许 MCP（L1 白名单）」描述的是**软边界**约束（详见 §6.2）：L1 tools 白名单在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证），dm-seek 全部 7 agent body 均超此阈值，故 L1 已降级为软边界。实在边界保障来自**声明层 + evidence-verifier 校验层**。每个 agent 定义须含固定区块「## 职责范围 / ## 允许使用的 MCP 服务 / ## 边界约束」。
...
### 6.2 关键归属约束（独占 = 软边界 / 两防线）

> **用户最终裁决（维持路径 B）**：共用 `.mcp.json` 可接受；但**每个 agent 必须显式声明「职责范围 + 允许使用的 MCP 服务」，明确禁止调用领域外 MCP 服务，边界清晰**。据此，本系统的「独占」由**两道实在防线**叠加构成。**L1 tools 白名单原为技术强制层，但 6/16 全项目评审 qa-engineer 用 CLI 2.1.177 实证：L1 在 agent body > ~40 行时不生效（dm-seek 全部 7 agent body 均超此阈值），已降级为软边界。**当前实在防线如下：

**两道防线**

1. **声明层（agent 定义固定区块，主防线之一）**：每个 agent 定义含三段固定区块——
   - `## 职责范围`：本 agent 的域与产出；
   - `## 允许使用的 MCP 服务`：枚举本域允许的 `mcp__` 服务/工具；
   - `## 边界约束`（硬性）：**禁止调用领域外 `mcp__` 工具**；跨域需求**经任务列表 / 消息向 owner agent 请求**（如 code-analyst 远端取码 → 请求 repo-tracer），**绝不直接调领域外 MCP**。
2. **校验层（evidence-verifier 运行期兜底，主防线之二）**：`evidence-verifier` 校验结论时，**标记「结论引用了声明范围外的工具 / 数据来源」的边界违规**，作为运行期兜底；与置信度校验同处一环。

**L1 降级为辅助参考**：各 agent frontmatter 的 `tools` 白名单仍按域编写（repo-tracer 含 `mcp__github-*`；jira-tracer 含 `mcp__jira__jira_get` 只读；kb-keeper 仅 KB/obsidian 工具；dongmei-ma / synthesizer / evidence-verifier / code-analyst 不含任何源类 `mcp__`），作为**设计意图文档**与声明层一致性的校验参照——但**不对引擎强制执行抱有依赖**。白名单在 body ≤ ~40 行的简单 agent 下生效，在 dm-seek 全部 7 agent 下不生效。

**逐项归属（在两防线下成立）**

- **远端 GitHub MCP 独占于 `repo-tracer`**：声明层明确禁调其余 agent；多仓场景下每 git repo 一个独立 GitHub MCP 实例（独立 token），均由 repo-tracer 管理与路由。**独占口径收窄（优化A）**：独占仅针对**远端 GitHub MCP**（取码 + 远端提交历史）；**本地 git 历史读取权 `code-analyst` 与 `repo-tracer` 共享、非独占**——本地 git 经 Bash 直读、无远端凭据风险，态B 本地非过时仓由 code-analyst 直读本地 git 片段附给 repo-tracer 收口（未附则 repo-tracer 自取）。
- **远端模式**下，`code-analyst` 需要的代码内容**与远端提交历史均经 `repo-tracer` 取得**（自身声明层禁调），以任务/消息请求实现。
- **跨仓查询**：由 `kb-keeper` 线索 + `code-analyst` 定位**映射到具体 repo + 模块**，再由 `repo-tracer` 路由到对应 GitHub MCP 实例 / 本地仓库。
- **Obsidian 知识库读写唯一收口于 `kb-keeper`**：声明层明确禁调其余 agent。**多 agent 增量发现（优化B）不破此独占**——code-analyst / repo-tracer / jira-tracer 只把增量发现作为产物字段上报（非 KB 写动作），由 dongmei-ma 终局归并交 kb-keeper 落库，写库仍唯一收口于 kb-keeper。
- `dongmei-ma` 协调与用户接口层，声明层明确不直连任何信息源；归并增量发现转交 kb-keeper 时**自身不写 KB**。

> **诚实声明（口径，必须随交付文档发布）**：本系统的「独占」= **每 agent 显式边界声明（声明层）+ evidence-verifier 校验（校验层）** 两者叠加，构成软边界。**L1 tools 白名单在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证，dm-seek 全部 7 agent body 均超此阈值）**——已降级为设计意图文档（与声明层一致性校验参照），**不对引擎强制执行抱有依赖**。**不得宣称「物理隔离 / L1 生效 / 三道防线」**。声明层 + 校验层保证边界**清晰、可审计**——即便某 agent 越界调了领域外 MCP，校验层在结论中标记边界违规、使其可被追溯。
...
- **独占=软边界（两防线）**：MCP 实例对全队共享加载，per-agent MCP 独占在当前 Claude Code **无可靠原生机制**——L1 tools 白名单在 agent body > ~40 行时不生效（6/16 qa-engineer 实证，dm-seek 全部 7 agent 均超此阈值）。实在边界保障靠**声明层**（每 agent 边界声明区块，硬性禁调领域外 MCP）+ **校验层**（evidence-verifier 标记边界违规）构成软边界。详细定义见 §6.2。
- **影响实现（T8 骨架）**：7 个 agent 定义放在 `.claude/agents/`，每个 ① `tools` 白名单作为设计意图文档（不依赖引擎强制执行），② 含 `## 职责范围 / ## 允许使用的 MCP 服务 / ## 边界约束` 三段固定区块（声明层）；MCP 配置占位统一进共享 `.mcp.json`；skills 统一进项目级 `.claude/skills/`；**不依赖** frontmatter 的 `mcpServers`/`skills`。`evidence-verifier` 实现含边界违规标记（校验层）。
...
| 依赖 | 类型 | 用途 | 凭据 / 备注 |
| --- | --- | --- | --- |
| 本地知识库（Obsidian Vault + Knowlery 插件） | 核心 | 代码模块定位线索；结论持久化 | 需初始化目录；边界唯一经 `kb-keeper` 读写（声明层 + 校验层，见 §6.2）；**Knowlery 是 Obsidian 插件、无独立编程 API**，集成走 obsidian CLI + Knowlery 技能 `/ask` `/cook` |
| **Jira MCP** | 核心 | 工单详情与业务原因 | 选型 **`@aashari/mcp-server-atlassian-jira`**（通用 HTTP 透传型）；需 JIRA URL / 邮箱 / API Token；配置于共享 `.mcp.json`，`jira-tracer` 声明层仅授权 **`mcp__jira__jira_get`（只读）** |
| **GitHub MCP（远端）** | 核心 | 非研发用户取代码 + 远端提交历史；远端模式代码来源 | 选型 **GitHub Copilot 托管 MCP**：`https://api.githubcopilot.com/mcp/`；**每个 git repo 配置一个独立实例（独立 GitHub token）**，全部实例配置于共享 `.mcp.json`，其 `mcp__github-*` 工具经 **`repo-tracer` 边界独占（仅远端，声明层 + 校验层，见 §6.2）** |
...
- 归属约束（**远端** GitHub MCP 独占 repo-tracer〔本地 git 共享，优化A〕、KB 读写独占 kb-keeper、dongmei-ma 不直连源等）经**软边界（两防线）**保证：**每 agent 显式边界声明 + evidence-verifier 校验**——属边界约束而**非物理隔离**；L1 tools 白名单在 agent body > ~40 行时不生效，已降级为设计意图文档，详见 §6.2 诚实声明。
...
- 角色定义（`.claude/agents/`，7 个首版 agent；每个含 ① `tools` 白名单（设计意图文档，L1 对 body > ~40 行不生效）+ ② `## 职责范围 / ## 允许使用的 MCP 服务 / ## 边界约束` 三段固定区块（声明层），实现软边界归属约束；**不依赖** frontmatter 的 `mcpServers`/`skills`）
...
- **文档与测试**（含软边界诚实声明：独占 = 声明层 + 校验层，**非物理隔离**，L1 在 body > ~40 行时不生效（6/16 实证），见 §6.2、附录 B）
...
- **独占 = 软边界 / 两防线**（用户最终裁决，维持路径 B）：**每 agent 显式边界声明**（`## 职责范围 / ## 允许使用的 MCP 服务 / ## 边界约束`，硬性禁调领域外 MCP）+ **evidence-verifier 校验层**（标记边界违规）。属边界约束、**非物理隔离**；**L1 tools 白名单在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证，dm-seek 全部 7 agent body 均超此阈值）**——已降级为设计意图文档，不对引擎强制执行抱有依赖；交付文档须诚实声明，**不得写「物理隔离/L1 生效/三道防线」**（见 §6.2、附录 B）。
...
- **L1 tools 白名单已实证不生效（v0.4.4）**：teammate 形态下 MCP 实例对全队共享加载，per-agent MCP 独占在当前 Claude Code **无可靠原生机制**——L1 tools 白名单在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证），dm-seek 全部 7 agent body 均超此阈值；`deniedMcpServers` 是组织/会话级一刀切、不能 per-agent（会误伤），不可用。**当前独占 = 软边界**：声明层（每 agent `## 边界约束` 硬性禁调领域外 MCP）+ 校验层（evidence-verifier 标记边界违规）使边界**清晰、可审计**。若未来 Claude Code 修复 L1 对复杂 agent 的支持，可恢复为三道防线。
- **白名单配置错误风险**：L1 白名单虽对复杂 agent 不生效，但声明层与白名单的一致性仍需维护——白名单条目误配领域外 `mcp__` 工具将暴露设计意图与声明层的矛盾，由 critic（T7）设计期校验白名单 + 声明层一致性，evidence-verifier 运行期兜底。
...
| 独占机制 | 未涉及 | v0.1~v0.3 隐含物理独占；v0.4 修订为策略级独占（tools 白名单）；**v0.4.1 经用户最终裁决定为「双层边界 / 三道防线」**：L1 白名单 + 每 agent 显式边界声明 + evidence-verifier 校验；**v0.4.2 独占口径收窄**：独占仅针对**远端 GitHub MCP**，**本地 git 历史读取权 code-analyst/repo-tracer 共享**（优化A）；**v0.4.4 L1 降级为软边界**：6/16 实证 L1 在 agent body > ~40 行时不生效（全部 7 agent 均超阈值），独占 = 声明层 + 校验层两防线，L1 退为设计意图文档 |
...
## 附录 B：运行形态与独占机制修订记录（subagent 假设 → teammate 形态 → 双层边界 → 软边界）
...
- **用户最终裁决 v0.4.1（独占口径，维持路径 B 不切 A）**：接受共用 `.mcp.json`；但要求**每个 agent 显式声明「职责范围 + 允许使用的 MCP 服务」、明确禁调领域外 MCP**。据此独占定稿为**双层边界 / 三道防线**（见 §6.2），不单押 L1。
- **v0.4.4 L1 降级（6/16 实证）**：qa-engineer 用 CLI 2.1.177 实证 L1 tools 白名单在 agent body > ~40 行时不生效，dm-seek 全部 7 agent body 均超此阈值。L1 降级为软边界（设计意图文档），实在防线为声明层 + 校验层。
...
> 推论：源类 MCP 对全队共享加载，独占无法靠物理隔离实现。L1 `tools` 白名单在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证），当前独占 = **声明层 + 校验层**软边界。若未来 Claude Code 修复 L1 对复杂 agent 的支持，可恢复为三道防线。

### B.3 L1 屏蔽机制状态（v0.4.4 更新：实证不生效）

**早先正面佐证（2026-06-12，CLI 2.1.165，已失效）**：以 `claude -p --agent repo-tracer --strict-mcp-config --mcp-config <临时配置>` 启动简单 agent 会话（body 短），可用工具集 = `[Bash, Read, SendMessage]`——证实 L1 白名单对**简单 agent（body ≤ ~40 行）**生效。

**2026-06-16 实证推翻（CLI 2.1.177，qa-engineer）**：对 body > ~40 行的复杂 agent（dm-seek 全部 7 agent 均在此范围），L1 tools 白名单**不生效**——agent 实际可用工具集超出白名单声明。此结果直接推翻 6/12 简单 agent 测试的推广结论。

**当前状态**：L1 白名单在 dm-seek 全部 7 agent 下**不构成技术强制**。已降级为**设计意图文档**——白名单条目体现设计期的"应然"约束，但引擎不强制执行。

**诚实声明硬性口径（v0.4.4）**：独占 = **每 agent 显式边界声明（声明层）+ evidence-verifier 校验（校验层）**，构成软边界。**L1 tools 白名单在 body > ~40 行时不生效，不得依赖。** **不得写「物理隔离 / L1 生效 / 三道防线」。** 若 future Claude Code 修复此问题，可恢复 L1 为实在防线。

**兜底**：声明层 + 校验层的软边界仍使归属约束**清晰、可审计**——违规调用可在结论中被标记和追溯。

### B.4 对实现的约束（v0.4.4 更新：L1 降级后的约束）

- 7 个 agent 定义置于 `.claude/agents/`，**每个含**：① `tools` 白名单（设计意图文档，不对引擎强制执行抱有依赖——L1 在 body > ~40 行时不生效）；② 固定声明区块 `## 职责范围 / ## 允许使用的 MCP 服务 / ## 边界约束`（声明层，硬性禁调领域外 `mcp__`、跨域经任务列表/消息向 owner agent 请求）。
  - `repo-tracer`：声明层允许 `mcp__github-*`（全部 GitHub MCP 实例工具，只读子集）；其余 agent 声明层禁调。
  - `jira-tracer`：声明层允许 `mcp__jira__jira_get`（**只读**，`@aashari` server 为通用 HTTP 透传型，见 KB「dm-seek Jira MCP 接口」结论）。
  - `kb-keeper`：仅 KB/obsidian 工具（obsidian CLI / Knowlery 技能，**非 `mcp__`**）；声明层禁调任何源类 `mcp__`。
  - `dongmei-ma` / `synthesizer` / `evidence-verifier`：声明层禁调任何源类 `mcp__`；`code-analyst` 仅本地代码读取工具。
...
- 此规范为 agent 实现的强制要求。L1 在 body > ~40 行时不生效（2026-06-16 qa-engineer 实证），依赖声明层 + 校验层构成软边界。
