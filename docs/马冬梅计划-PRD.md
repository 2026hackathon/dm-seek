...
7. **synthesizer** 综合 code + git + jira 三源，产出**双层结论**：① **`executiveSummary`**（面向非技术人员的自然语言结论摘要，简明扼要的"发生了什么、为什么、影响是什么"）；② **完整结论**（按场景的结构化分析，含证据链、偏差注记、场景特定评估）。三源/KB 与代码冲突时以代码为准、记为「记录与实现的偏差」。
...
- **`executiveSummary`**（非技术人员可读的结论摘要：发生了什么、为什么、影响是什么）
- **当前实现状态**（代码现实）
- **演变时间线**（Git，含关联工单号）
- **根因解释**（Jira 业务原因）
