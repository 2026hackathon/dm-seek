...
> **v0.4.4 更新**：2026-06-16 qa-engineer 用 CLI 2.1.177 实证 L1 `tools` 白名单在 agent body > ~40 行时不生效。L1 降级为设计意图文档，不对引擎强制执行抱有依赖。实在保障来自以下两防线。

- **声明层**：每个 agent 的「允许使用的 MCP 服务」声明区块不变。
- **校验层**：evidence-verifier 的边界违规校验不变。
- **L1 白名单**（v0.4.4 降级）：OAuth/PAT 路径均不改变 engine 行为——无论何种认证路径，`tools` 白名单在 body > ~40 行的 agent（dm-seek 全部 7 agent）上不生效。白名单配置保留作为设计意图文档，不宣称技术强制。
