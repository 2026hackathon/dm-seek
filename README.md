...
- **信息源独占**：GitHub MCP 仅 repo-tracer 可调用（远端取码由 code-analyst 请求 repo-tracer 中转）、Jira MCP 仅 jira-tracer 可调用（且只读）、KB 读写仅 kb-keeper 可操作——靠声明层（每 Agent 边界声明）+ 校验层（evidence-verifier 校验）两防线构成软边界
...
3. 授权完成后，`mcp__atlassian__search_issues` 和 `mcp__atlassian__get_issue` 自动可用

##### Obsidian 知识库

1. 安装 [Obsidian](https://obsidian.md/zh/)
2. 用 windows-setup.ps1 Phase 4 创建 KB vault（自动注册到 Obsidian）
...
> **诚实声明**：`tools` 白名单在各 agent frontmatter 中保留作为设计意图文档，但**在 agent body > ~40 行时不生效**（2026-06-16 qa-engineer 用 CLI 2.1.177 实证，dm-seek 全部 7 agent body 均超此阈值）。白名单不构成技术强制——不对引擎强制执行抱有依赖。
...
- **声明层 + 校验层构成软边界**：每 Agent 的边界声明区块 + evidence-verifier 运行期边界违规校验，保证边界清晰、可审计。`tools` 白名单保留作为设计意图文档，但 L1 在 agent body > ~40 行时不生效（2026-06-16 qa-engineer 用 CLI 2.1.177 实证），不依赖引擎强制。
