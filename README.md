...
| 防线 | 层级 | 机制 | 作用 |
|------|------|------|------|
| **L1 技术层** | 硬约束 | 各 Agent `tools` 白名单精确收口——仅 repo-tracer 含 `mcp__github__*`、仅 jira-tracer 含 `mcp__atlassian__search_issues` + `mcp__atlassian__get_issue`（只读）、其余 Agent 不含源类 `mcp__` | 阻止非授权 Agent 调用源 MCP 工具 |
