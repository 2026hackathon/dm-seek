---
name: synthesis-core
description: 马冬梅计划三源综合分析方法库（单 skill 多 method）——综合六步骨架(三源对齐/时间线编织/结论生成/出处挂接/矛盾标记/自检交棒) + 9 类场景特化 method（质量评估/影响范围/缺陷责任/onboarding/技术债/回归溯源/功能蒸发/接口仲裁/设计对齐）。以代码为锚、每条结论挂出处、缺源入 unknowns 不空想。供 synthesizer 综合 code+git+jira 时使用；evidence-verifier 可复用其判据。
---

# synthesis-core — 综合分析方法库（六步骨架 + 9 场景 method）

> 本 skill 是 synthesizer 分析方法的**单一权威载体**，固化自 `docs/design-synthesis-and-verification.md` 第一部分。所有 9 类场景共享**通用六步骨架**，差异只在「侧重维度」与「特化步骤」。**单 skill 多 method**：9 个场景方法以 method 片段收于本 skill 场景库（§2），避免碎片化。输出对齐契约 `docs/design-agent-io-schema.md` §2.7（`synthesis` 载荷）。

## 何时用

- synthesizer 收齐 code-analyst(`code_location_set`) + repo-tracer(`repo_timeline`) + jira-tracer(`jira_reasons`) 三源，要综合出结论时。
- 据 dongmei-ma `query_plan.intent`/`scenario` 选 method（§2）；intent 未明或跨场景 → 用通用六步，`analysisMethod=synthesis-core-generic-v1`。

## 0. 硬约束（贯穿全流程，PRD §1.2）

- **以代码为唯一事实基准**：三源冲突时按代码事实陈述，把冲突另一方记为「记录与实现的偏差」，不和稀泥。
- **每条结论必须挂出处**（`evidence[]`，code/commit/jira）；无出处的判断移入 `unknowns`，**不冒充结论、不空想补源**——补源靠返工循环（契约 §7），不在本环节。
- 缺的维度进 `unknowns`，交 evidence-verifier 判置信度。

## 1. 通用六步骨架（`synthesis-core`，每次综合恒定执行）

| 步 | 名称 | 动作 | 产出落点（契约 §2.7） |
| --- | --- | --- | --- |
| S1 | 三源对齐 | 把 `code_location_set` / `repo_timeline` / `jira_reasons` 按 **代码实体 ↔ commit ↔ 工单** 三方关联对齐，**以代码实体为锚**（PRD §1.2）。逐 location 找触碰它的 commit（按 `touchedPaths`），逐 commit 找其 `ticketIds` 对应的 jira 业务原因。 | `sourcesPresent{code,git,jira}` |
...
| S6 | 自检交棒 | 填 `analysisMethod`（method id）、`sourcesPresent`、`unknowns`，交 evidence-verifier。 | 整个 synthesis 载荷 |

> **以代码为锚（S1/S5 硬约束）**：Jira 描述/记忆与代码现状冲突时，结论按代码事实陈述，把冲突另一方记为「记录与实现的偏差」。

## 2. 九类场景 method 库（method id 命名 `<scene-slug>-v1`）

每类 = 通用六步 + 侧重维度 + 特化步骤（在 S2/S3 注入）。`analysisMethod` 取对应 method id。

| # | 场景 (PRD §5) | method id | 主侧重维度 | 特化步骤 | 三源关键依赖 |
| --- | --- | --- | --- | --- | --- |
...
> 沉淀决策：**`synthesis-core` 是唯一强制 skill**；9 场景 method 收于同一 skill 场景库（单 skill 多 method，类比 core-ng 规则集中一处）。扩展新场景 = 在本表加一行 method。场景 9 为二期 stub，调用即返回「二期未实现」提示，不破坏场景库结构。
...
- 任一场景三源不全时**不空想**：缺的维度进 `unknowns`，由 verifier 判置信度并可能触发返工——本骨架不负责补源（补源靠契约 §7 返工循环）。

## 4. evidence 组织规范（每条结论挂出处）

- 每条 `conclusion` 的 `evidence[]` 用统一出处对象（契约 §2.5）：`{type, ref}`，`type ∈ {code, commit, jira, kb}`，`ref` 为可定位引用（`路径#行` / `repo@sha` / `工单号` / `KB路径#行`）。
- synthesizer 产 evidence **不含直接 MCP 调用结果**：code 来自 code-analyst 的 `code_location_set`、commit 来自 repo-tracer 的 `repo_timeline`、jira 来自 jira-tracer 的 `jira_reasons`（边界约束，契约 §5）。
- `sourcesPresent{code,git,jira}` 据实际挂上的出处类型如实填写（verifier 置信度判据的直接输入）。

## 5. unknowns 标注规范

进 `unknowns` 的情形：① S4 中无出处的判断；② S5 中三源冲突未解的另一方；③ 场景关键维度缺源（如 root_cause 场景缺 jira 业务原因）。每条 unknown 文本应可被 verifier 映射到缺环（`code_location` / `git_timeline` / `jira_reason` / `repo_coverage` 等，见 verifier §6 缺口格式）。

> 权威依据：`docs/design-synthesis-and-verification.md`（第一部分 §1~3）、`docs/design-agent-io-schema.md`（§2.7 synthesis / §5 边界 / §7.3 发散清单）。
