...
> 本 skill 是 synthesizer 分析方法的**单一权威载体**，固化自 `.claude/rules/design-synthesis-and-verification.md` 第一部分。所有 9 类场景共享**通用六步骨架**，差异只在「侧重维度」与「特化步骤」。**单 skill 多 method**：9 个场景方法以 method 片段收于本 skill 场景库（§2），避免碎片化。输出对齐契约 `.claude/rules/design-agent-io-schema-reference.md` §2.7（`synthesis` 载荷）。
...
> 权威依据：`.claude/rules/design-synthesis-and-verification.md`（第一部分 §1~3）、`.claude/rules/design-agent-io-schema-reference.md`（§2.7 synthesis / §5 边界 / §7.3 发散清单）。
