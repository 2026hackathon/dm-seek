---
name: setup-guide
description: 马冬梅计划开箱引导/配置参考手册——GitHub 双轨认证、repos.json 骨架、故障排查。Windows 用户主入口为 scripts/windows-setup.ps1，本文为补充参考。
---

# setup-guide — 配置参考手册

> **Windows 用户**：主入口是 `scripts/windows-setup.ps1`（交互式分步菜单，按需选择操作）。本文为补充参考：配置骨架说明、安全铁律、故障排查、macOS 手动步骤。

## 与 windows-setup.ps1 的分工

| 内容 | windows-setup.ps1 | 本手册 |
|------|:---------:|:------:|
...
- **终端历史泄漏防范**：Windows `Read-Host -MaskInput`（`windows-setup.ps1` 已内置），macOS/Linux `read -s`

---

## .mcp.json 双模式

windows-setup.ps1 根据认证路径自动生成。手动编辑时参考以下示例：
...
| `kb` | 否 | KB vault 配置，由 windows-setup.ps1 Phase 4 自动写入。无此字段 = KB 未初始化 |
...
windows-setup.ps1 可重复运行，支持增量更新：新增仓库不覆盖已有条目、已有仓库补缺仅补充缺失字段、覆盖已有字段需用户确认。

---

## macOS 手动步骤

windows-setup.ps1 仅支持 Windows。macOS 用户参照以下最小步骤：
...
| 症状 | 可能原因 | 解决 |
|------|---------|------|
| `/mcp` 中 github server 不显示 | .mcp.json 未配置 | 运行 windows-setup.ps1 [5] |
...
| 症状 | 可能原因 | 解决 |
|------|---------|------|
| Agent 启动报 "repos.json 为空" | 未运行仓库配置 | 运行 windows-setup.ps1 [3] |
...
| 症状 | 可能原因 | 解决 |
|------|---------|------|
| kb-keeper 报 "KB 未就绪" | repos.json 无 `kb` 字段 | 运行 windows-setup.ps1 [4] |
| Obsidian CLI 未找到 | 环境变量未设 | windows-setup.ps1 Phase 6 自动设置；macOS 手动设 `DMSEEK_OBSIDIAN_CLI` |
