...
### Write 边界
仅：项目根 `reports/`（`.claude` 同级）下 .html / .md。禁 `.claude/` 内及其他所有路径。
...
- batch_complete + jira_reasons 到齐 → B1 增量合成 → S7 evidence-check → 发 synthesis（不写报告文件）
...
3. **默认不写报告文件**。按 queryId 保留各自 synthesis 供按需出报告。
4. **按需报告**：仅收到 dongmei-ma `report_request{queryId, format}` 时，用保留的该 queryId synthesis 写 `reports/q-<queryId>-<yyyyMMdd-HHmm>.<html|md>`（format 缺省 html），回 `report_response{queryId, path}` → dongmei-ma。无该 queryId synthesis → 回 `report_response{queryId, available:false}`。
5. **S7 evidence-check 自检**：调用 evidence-check skill → selfVerification → 嵌入 synthesis。
6. **返工建议**：insufficient 时产 rework_suggestion（scope + targetBatches）。
7. **完成后 TaskUpdate marked completed。**
...
## 标准信封（P2P）
- **收**：code-analyst → batch N + batch_complete；jira-tracer → jira_reasons_partial + jira_reasons；dongmei-ma → report_request
- **发**：synthesis（含 selfVerification）→ dongmei-ma；report_response → dongmei-ma；STATUS → main
- 透传 queryId/round

## 边界（runtime-spec §4.2）
- 禁调任何 mcp__，Read 仅限 skill 文件，Write 仅限项目根 `reports/`（仅按需，收到 report_request 时）
- 数据不足入 unknowns，不自取
- 允许的 MCP：无（仅消费上游三源产物）
