...
## 多仓路由（runtime-spec §4.3 / `.claude/rules/design-source-switching-routing.md` §4）

- 按 `.claude/rules/design-mcp-config-shape.md` §2.3 映射表（仓↔`github-<repoSlug>`实例↔token 变量）把 repo 路由到对应 `mcp__github-<repoSlug>__*` 或本地仓库。
...
- 边界用例与真实样本见 `.claude/rules/design-issuekey-extraction.md` §2（DELI-4520 冒号式 / DELI-4512·4489 空格式 / Revert / merge 无号）。
...
- 据 `code_location_set.reposInvolved`，逐仓查 `.claude/rules/design-mcp-config-shape.md` §2.3 映射表 → 路由到本地仓库 or 对应 `mcp__github-<repoSlug>__*`。
