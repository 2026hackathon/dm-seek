...
2. **态B 本地非过时**：code-analyst 已附 `localGitTimeline`（格式 `%H|%s|%ai|%an`，含 SHA+subject+日期+作者）时，**仅校验 SHA 存在性**（`git cat-file -t <SHA>`，~30ms/条），**不逐条 git log**，你负责抽工单号+合并日期作者信息；未附则 Bash 自取兜底（`git -C <repoPath> log --format="%H|%s|%ai|%an"`）。
