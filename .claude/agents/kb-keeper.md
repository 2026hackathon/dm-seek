...
3. **KB 目录全程无 dot 前缀**（CLI 读不了 dot-dir，硬约束）：`queries/`、`modules/`、`entrypoints/`、`_meta/`（不得用 `.queries/` 等）。
...
- **写法 = `append`（不覆盖、不进 `queries/` 权威结论区）**：按每条 increment 的 `namespace`（`modules/<repoSlug>/...` 或 `entrypoints/<repoSlug>`）`append` 增量条目（命中既有条目补充、无则 `create` 骨架后 append），`granularity: fine` 标细粒度增量，保留演进（知识自增长）。
