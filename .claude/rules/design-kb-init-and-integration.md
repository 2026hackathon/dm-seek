...
> 本文是 KB 初始化流程与 kb-keeper 集成形态的**权威契约**：建库遍历算法、多仓与双源代码来源、kb-keeper 唯一读写口的工具映射、KB 目录结构与沉淀 SCHEMA、知识自增长与中英摘要落库形式。
...
6. **职责边界铁律**：KB-init 是一个 **skill**，不是一个 agent；它在 dongmei-ma 编排下调度既有 7 agent，**写库动作一律经 kb-keeper**，不旁路。
...
> KB-init skill 自身不读源码、不连 MCP、不写库；它是 dongmei-ma 驱动的一段**编排流程**，各专职动作落到对应 agent。这保证「KB 读写独占 kb-keeper」「GitHub MCP（远端）独占 repo-tracer」「dongmei-ma 不直连源」三条约束在建库路径上同样成立。本地 git 历史读取权 code-analyst/repo-tracer 共享，独占只针对远端 GitHub MCP。

---

## 2. KB 初始化流程（KB-init）

### 2.1 建库种子 = core-ng 入口点
...
> 识别规则的权威定义以 `design-core-ng-recognition.md` 为准；本文只消费「入口点 + 调用链」这一遍历能力，不重复定义识别规则。
...
- 跨服务隐性调用可能跨仓——建库阶段按「显式调用链」覆盖，跨仓隐性依赖由查询期 evidence-verifier 的发散返工兜底。
...
> 建库期不做「过时判定」（那是查询期、按被检索代码段粒度的事）；建库就以当前可得版本为基线，并把 commit HEAD 记入 init 元数据，便于后续判定。
...
1. **二进制位置**：`D:\obsidian\Obsidian.com`（Windows）。kb-keeper 不能假设其在 PATH 中——**需先刷新/显式指定路径**。macOS 路径不同，由引导 skill 在配置期探测并写入 kb-keeper 可读的配置（如 `${DMSEEK_OBSIDIAN_CLI}` 环境变量占位）。
2. **PATH 刷新**：新装/新会话可能 PATH 未刷新；kb-keeper 调用前应能容错「命令未找到」并提示走显式路径。
3. **不可读 dot-dir**：obsidian CLI **读不了以 `.` 开头的目录**。
   - **(a) vault 内部目录命名**：KB vault 目录结构（§4）**一律不使用 dot 前缀目录**（如不能用 `.queries/`，用 `queries/`）。这是目录设计的硬约束。
   - **(b) vault 根路径选址**：若 **vault 根目录本身、或其路径任一父段以 `.` 开头**（如 `~/.obsidian-vault/`），整个 vault 都会被 obsidian CLI 读不到。⇒ **引导 skill 在探测/确认 vault 路径时须校验**：发现 vault 根或任一父段以 `.` 开头 → 提示用户「该路径 obsidian CLI 不可读，请改用非 dot 路径」。
...
> Knowlery skill 放项目级 `.claude/skills/`；kb-keeper 经 `Skill` 工具在运行时调用。「KB 读写独占 kb-keeper」靠 kb-keeper 之外的 agent `tools` 不含 KB 写路径。

---

## 4. KB 目录结构（沿用 vault 既有约定 + 本系统命名空间）

> 目录结构沿用 vault 既有约定，结论写入 `queries/`。本系统在此之上加 repo 命名空间与建库分区。**所有目录无 dot 前缀**（§3.2 约束）。

```
<vault-root>/
  queries/                      # 结论沉淀区；查询结论 + 建库产出的可查条目
...
[查询主流程末端，evidence-verifier 判“充分”后]
  ...
  kb-keeper /cook：把本次结论按 QUERY-SCHEMA 编译进 queries/<repoSlug>/<slug>.md
       ...
       - 挂出处：code(file:line)/commit/工单号（每条结论可回挂）
...
- **降级交付条目**（`degraded: true`）：返工 2 轮仍证据不足而降级交付时，结论**仍写 KB 留轻量记录**（便于追溯与后续补全），但 `degraded: true` 标记 + `confidence` 多为「低」/「中」。**`/ask` 命中 `degraded:true` 条目时不得当权威秒答**——须提示「此为证据不足的降级结论，建议重新溯源/补证」，由 kb-keeper 在 `/ask` 检索逻辑中据此字段过滤或降权。
