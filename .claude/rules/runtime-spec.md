...
5. **repo-tracer** 统一收口**提交时间线**（态B 采用 code-analyst 提供的本地 git 片段、未附则自取，远端经 GitHub MCP），并从 commit subject **抽取 Jira 工单号**。
   ...
   > `code_location_set.localGitTimeline` 格式：code-analyst 附带 `git log --format="%H|%s|%ai|%an"` 完整输出（每行 `SHA|subject|date|author`）。repo-tracer 仅需校验 SHA 存在性（`git cat-file -t <SHA>`），信任 code-analyst 已覆盖相关文件范围。
...
| `chunkInfo` | object | 可选 | 分片时携带（见下方「分片通信规则」） |
...
- **归并职责**：**dongmei-ma 是唯一分片归并点**。缓存 key = `queryId + chunkId`（内存暂存），每收一片将列表字段 append 到缓存；**收到末片（`chunkIndex===totalChunks-1`）后立即合并全部缓存分片为完整 payload 转发下游，不做额外判断。**下游 agent（synthesizer / evidence-verifier）见到的是合并后的完整 payload。
