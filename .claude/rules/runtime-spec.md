...
MCP server 经项目 `.mcp.json` 注册；`teammateMode: in-process` 下 teammate 工具 = lead spawn 时已就绪工具 ∩ teammate allowlist（就绪门控见 §4.6）。各 teammate 由 frontmatter allowlist 收窄，互不越界。「独占」靠两防线构成软边界：
...
**启动（硬约束）**：以普通 `claude` + `/dm-start` 启动；禁用 `claude --agent dongmei-ma`。

**前置条件（两者缺一即 teammate 无 MCP）**：①lead 为普通会话（非 `--agent`）；②spawn 前 MCP 已 connected。teammate 工具 = lead 在 spawn 那刻已就绪工具的快照 ∩ teammate allowlist，事后不补发。

**MCP 就绪门控（硬约束，`teammateMode: in-process`）**：dongmei-ma 按序执行，前一步未过禁止下一步：
1. spawn 前调 `mcp__github__get_me` + `mcp__plugin_atlassian_atlassian__getAccessibleAtlassianResources` 探活（只读、丢弃结果，§4.4 与 dongmei-ma §0.3 唯一例外）；任一失败按 2-3s 重试至多 5 次。
2. 仍失败 → 不 spawn，提示用户 `/mcp` 认证后重跑 `/dm-start`。
3. 均成功 → spawn 5 个 worker。
4. spawn 后令 git-tracer 调 `mcp__github__get_me`、jira-tracer 调 `mcp__plugin_atlassian_atlassian__getAccessibleAtlassianResources` 各一次交叉确认；任一不可用 → 终止建团、提示重启，不进入查询。
