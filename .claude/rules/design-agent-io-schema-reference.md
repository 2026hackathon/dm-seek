...
repo-tracer 是 **Git/GitHub 网关，独占全部 GitHub MCP 实例（远端）**（PRD §6.2）。本地 git 历史读取权与 code-analyst 共享——**态B 本地非过时仓优先采用 code-analyst 随 `code_location_set.localGitTimeline` 附来的本地 git 片段（不重复跑 git log），未附则经 `Bash` 自取兜底**；远端经 GitHub MCP（独占）。无论来源，`repo_timeline` 由 repo-tracer **统一收口产出**，**始终从 commit subject 抽取 Jira 工单号**（默认正则 `^([A-Z]+-\d+)[:\s]`，冒号或空格分隔，本仓 `DELI-\d+`，可配置，容错无号，Revert 提交穿透抽出原号，PRD O3 / `design-core-ng-recognition.md` §5）。
...
```json
{
  ...
  "payload": {
    "executiveSummary": "📋 订单超时取消配置发生过一次变更：\n\n2025年8月14日（DELI-4521），订单超时取消阈值从30分钟改为15分钟。变更原因是运营提出缩短库存占用的需求（DELI-4400派生）。\n\n当前代码中，OrderTimeoutPolicy 类的 thresholdMinutes 字段值为15（order-service 模块），相关逻辑通过 OrderCancelService.updateTimeouts() 分发。此次变更涉及1个commit、1个Jira工单，改动范围可控。\n\n**一句话结论：订单超时阈值已由30分改为15分，原因为缩短库存占用。**",
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `executiveSummary` | string | 必填 | **面向非技术人员的自然语言结论摘要**：用业务语言陈述发生了什么、为什么、影响是什么，引用关键证据但不展开完整推导。帮助产品经理/测试快速理解结论，再决定是否深入看 `conclusions[]` 全部细节 |
